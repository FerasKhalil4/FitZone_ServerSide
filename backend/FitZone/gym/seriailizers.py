...
class GymSerializer(serializers.ModelSerializer):
    ...
    manager = UserSerializer(write_only=True , required = False)
    woman_hours_details = WomanHoursSerializer(read_only=True , required=False)
    ...
    class Meta:
        model = Gym
        fields =['id','name' , 'description','regestration_price','image_path',
                 'created_at','start_hour','close_hour' ,'mid_day_hour','manager',
                 'manager_id','branch','woman_hours_details','woman_hours',
    ...
    def create(self, validated_data):
        if not validated_data.get('manager') and not validated_data.get('manager_id') :
...
class EmployeeSerializer(serializers.ModelSerializer):
    ...
    shifts = ShiftSerializer(read_only = True)
    ...
    class Meta:
        model = Employee
        fields = ['id','is_trainer','start_date','quit_date','shift','shifts','user','user_details']
